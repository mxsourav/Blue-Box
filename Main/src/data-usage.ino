 void datausage(){
  ...
  snprintf(buf, sizeof(buf), "RAM: %d%% used", ramUsage);
  u8g2.drawStr(4, 21, buf);

  // Box 2 - Flash
  u8g2.drawFrame(0, 26, 128, 12);
  snprintf(buf, sizeof(buf), "Flash: %d%% used", flashUsage);
  ...
  snprintf(buf, sizeof(buf), "Cores: %d R:%d", chip_info.cores, chip_info.revision);
