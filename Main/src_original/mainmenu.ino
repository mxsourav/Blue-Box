...
void drawmainMenu() {
  Serial.println("[DBG] drawmainMenu started");
  u8g2.clearBuffer();
  u8g2.setBitmapMode(1);
  u8g2.setFontMode(1);

  // Smooth interpolation towards selectedmainmenuIndex
  float diff = selectedmainmenuIndex - smoothScrollPos;
  // Handle wrapping around MENU_LEN smoothly
  if (diff > MENU_LEN / 2.0f) {
    smoothScrollPos += MENU_LEN;
  } else if (diff < -MENU_LEN / 2.0f) {
    smoothScrollPos -= MENU_LEN;
  }
  diff = selectedmainmenuIndex - smoothScrollPos;
  smoothScrollPos += diff * 0.25f; // Interpolation factor (0.25 is smooth at 33Hz)

  // Keep smoothScrollPos in [0, MENU_LEN)
  if (smoothScrollPos >= MENU_LEN) smoothScrollPos -= MENU_LEN;
  if (smoothScrollPos < 0) smoothScrollPos += MENU_LEN;

  u8g2.drawXBMP(127, 0, 1, 64, image_scroll_indicator_bits);
  u8g2.drawXBMP(0, 20, 122, 23, image_selection_bar_bits);

  // Draw position box at top-right corner (above scroll indicator) - smooth scrollbar
  int boxHeight = 5;
  float constrainedScroll = constrain(smoothScrollPos, 0.0f, (float)(MENU_LEN - 1));
  int boxY = (int)(constrainedScroll * (64 - boxHeight) / (MENU_LEN - 1));
  u8g2.drawBox(125, boxY, 3, boxHeight);

  // Draw items around the center item
  int centerIndex = (int)(smoothScrollPos + 0.5f);
  Serial.printf("[DBG] centerIndex=%d, smoothScrollPos=%f\n", centerIndex, smoothScrollPos);
  for (int i = -2; i <= 2; i++) {
    int itemIndex = (centerIndex + i + MENU_LEN) % MENU_LEN;
    
    float itemOffset = itemIndex - smoothScrollPos;
    if (itemOffset > MENU_LEN / 2.0f) itemOffset -= MENU_LEN;
    else if (itemOffset < -MENU_LEN / 2.0f) itemOffset += MENU_LEN;
    
    int yOffset = 20 + (int)(itemOffset * 22.0f);
    int textY = yOffset + 14;
    int iconY = yOffset + 3;

    if (yOffset > -22 && yOffset < 64) {
      Serial.printf("[DBG] Drawing itemIndex=%d name=%s\n", itemIndex, menuItems[itemIndex]);
      // Highlight the item physically closest to the center
      bool isSelected = (itemIndex == centerIndex);
      ...
      if (icon == NULL) {
        Serial.printf("[DBG] ERROR: Icon array is NULL for itemIndex=%d\n", itemIndex);
      }
  ...
  Serial.println("[DBG] sending buffer");
  u8g2.sendBuffer();
  Serial.println("[DBG] buffer sent");
}



void handleMenuAction(int index) {
  Serial.printf("[DBG] handleMenuAction index=%d\n", index);
  switch (index) {
    case 0:
...
void handlemainmenu() {
  Serial.println("[DBG] handlemainmenu started");
  gotoMainMenuFlag = false; // Reset the escape flag
...
void handlewifimenu() {
  ...
  const char* wifiItems[] = {"SCAN NETWORKS", "DEAUTH FLOOD", "PACKET SNIFFER"};
  const int wifiItemCount = 3;
  
  // Draw WiFi menu
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_7x14_tf);
  u8g2.drawStr(24, 12, "WiFi Tools");
  ...
  for (int i = 0; i < wifiItemCount; i++) {
    ...
    if (i == wifiMenuSel) {
      ...
      u8g2.setDrawColor(0);
      u8g2.drawStr(10, y + 11, wifiItems[i]);
      u8g2.setDrawColor(1);
    } else {
      u8g2.drawStr(10, y + 11, wifiItems[i]);
    }
  ...
  if (millis() - lastInput > 150) {
    if (digitalRead(BTN_UP) == LOW) {
      wifiMenuSel = (wifiMenuSel - 1 + wifiItemCount) % wifiItemCount;
      lastInput = millis();
    }
    if (digitalRead(BTN_DOWN) == LOW) {
      wifiMenuSel = (wifiMenuSel + 1) % wifiItemCount;
      lastInput = millis();
    }
    if (digitalRead(BTN_SELECT) == LOW) {
      ...
      switch (wifiMenuSel) {
        case 0: // Scan Networks → Scanner Loop
          wifiDoScan();
          // Enter scanner loop (BACK exits to this menu)
          while (digitalRead(BTN_BACK) == HIGH) {
            wifiScannerLoop();
            delay(1);
          }
          delay(200); // debounce BACK
          wifiCleanup();
          break;
          
        case 1: // Deauth Flood → Scan first, then flood
          wifiDoScan();
          deauthFlood();
          break;
